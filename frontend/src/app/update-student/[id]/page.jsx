...
const client = useClient(); // Get the client
  useEffect(() => {
    const fetchStudent = async () => {
      ...
      try {
        const response = await client.request(GET_STUDENT_BY_ID_QUERY, {
          id: studentId,
        });
        setOriginalData(response.getStudentById);

        // Set form values
        for (const key in response.getStudentById) {
          setValue(key, response.getStudentById[key]);
        }
      } catch (error) {
...
if (client) fetchStudent(); // Fetch the student
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await client.request(UPDATE_STUDENT_MUTATION, {
        id: studentId,
        ...formattedData,
      });
